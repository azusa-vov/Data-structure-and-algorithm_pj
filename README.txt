...
在作业中要求用
$ .\loganalyzer<test/test_commands.txt
实现读取命令并测试，但：

需要注意！！！由于PowerShell中"<"没有实现重定向，所以这个命令会报错，只需要在cmd中使用指令替代即可：
$ cmd /c "loganalyzer.exe < test_commands.txt"

所有待测试文件可以在/test目录底下找到
当然，也可以采用手动编译的方法：
...
(需要换成本地的相关路径)
随后在交互状态下LOAD相关文件即可进行测试。
$ ./loganalyzer

这里需要注意！！！
1.本项目的测试文件都已经备份于\test文件夹下，需要在LOAD时附上此路径（索引test_commands.txt时同理）：
（示例：LOAD test\test1.txt）

*2.关于内存占用和泄露：我在本地wsl使用valgrind工具进行验证，其中LOAD test3内存占用小于200MB的测试也通过
（结果截图一并附在压缩包），实际占用了34,366,370 bytes（约等于32.8MB<200MB）。关于具体指令是否泄露内存，
可以通过对/valgrind/test_valgrind.txt使用：
