...
void LinkedList::push_back(int id,std::string t, std::string l, std::string m, std::string msg) {
      LogNode* newNode = new LogNode(id,t, l, m, msg);//新定义一个结点
      if (head==nullptr) {
            head=newNode;
...
//尾插
void LinkedList::push_front(int id,std::string t, std::string l, std::string m, std::string msg){
      LogNode* newNode = new LogNode(id,t, l, m, msg);//新定义一个结点
      if (head == nullptr) {
            head = newNode;
            tail = newNode; //如果只有一个新插入的结点（原来是空），头尾都指向它
...
LinkedList* LinkedList::clone(){
      ...
      LogNode* curr=head;
      while(curr!=nullptr){
            newList->push_back(curr->id,curr->time,curr->level,curr->module,curr->message);
            curr=curr->next;
      }
      return newList;
}
